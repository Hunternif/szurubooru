...
def get_post(ctx: rest.Context, params: Dict[str, str]) -> rest.Response:
    ...
    if post.safety == model.Post.SAFETY_SKETCHY:
        auth.verify_privilege(ctx.user, "posts:view:sketchy")
    elif post.safety == model.Post.SAFETY_UNSAFE:
...
def get_posts_lookalikes(
    ...
    lookalikes = posts.search_by_signature(sig, limit + 1, threshold, query_text)
